...
import { updateBio } from "@/services/profile/profile-service";

export const runtime = "nodejs";

const bioSchema = z.object({
  bio: z.string().max(500, "La bio ne peut pas dépasser 500 caractères.").nullable(),
});
...
export async function PATCH(request: Request) {
  try {
    ...
    const input = bioSchema.parse(body);

    await updateBio(currentUser.id, input.bio);
