...
export function CreatePresetDialog({ open, onClose, onCreate }: CreatePresetDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-surface border border-border rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-lg font-semibold text-text-primary mb-4">Nouveau preset</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="preset-name" className="block text-sm font-medium text-text-secondary mb-1.5">
              Nom du preset
            </label>
            <input
              id="preset-name"
              ...
              maxLength={60}
              placeholder="Ex: Configuration stream"
